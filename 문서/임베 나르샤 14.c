...
void loop() // 3d 프린터 개조보드를 이용한 조작
{
	if(toggle_1 == 0){
		if(end_Y == 1){
			end_X = 0;
			end_Y = 0;
			TIMER1_ON(1, 200, 5000);
			TIMER3_ON(1, 400, 2500);
			toggle_1 = 1;
		}
	}
	if(toggle_1 == 1){
	...
	if(toggle_1 == 2){
		if(end_X == 1){
			end_Y = 0;
			TIMER3_ON(0, 400, 2500);
			toggle_1 = 0;
		}
	}
}

void TIMER1_ON(int TIMER1_toggle, int TIMER1_speed, int TIMER1_len){
	if(TIMER1_toggle == 0)
	{
		digitalWrite(X_DIR_PIN, HIGH);
		len_X = TIMER1_len;
...
void TIMER3_ON(int TIMER3_toggle, int TIMER3_speed, int TIMER3_len){
	if(TIMER3_toggle == 0)
	{
		digitalWrite(Y_DIR_PIN, HIGH);
		len_Y = TIMER3_len;
