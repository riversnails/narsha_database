...
double currunt_x = 0; // 현재 좌표 && mm 단위로 봄
double currunt_y = 0;
...
void goXLocation(double x, int speed)
{
	...
	int x_len;
	x_len = abs(currunt_x - x);

	x_move(x_len * ONE_MM, dir, speed);
	currunt_x = x;
}
...
void goYLocation(double y, int speed)
{
	...
	int y_len;
	y_len = abs(currunt_y - y);

	y_move(y_len * ONE_MM, dir, speed);
	currunt_y = y;
}

void goXYLocation(double x, double y)
{
...
void goLenHypo(int len, int x_dir, int y_dir, int degree) // 길이와 뱡항을 넣어 각도를 구해 가는 코드
{
	...
	x_move(x_len, x_dir, x_speed);
	y_move(y_len, y_dir, y_speed);
